...
const HomeBlogs = () => {
  ...
  return (
    <div className="box-border mx-auto w-[1200px] max-2xl:w-[1200px] max-xl:w-[980px] max-lg:w-full max-md:w-full max-sm:w-full px-5 lg::px-8">
      <SwiperComponent spaceBetween={25} slidesPerView={3} pagination={{ type: "bullets", clickable: true }} autoplay={{ delay: 2500, disableOnInteraction: false }} loop={true} modules={[Autoplay, Navigation, Pagination]}>
...
export const SingleBlogCard = (props) => {
  ...
  return (
    <Link href={`/pages/blogs/${blog?._id}`}>
      <div className="rounded-md bg-gray-50  md:p-5 lg:p-6">
        <div className="flex justify-center items-center text-white bg-primary-500 rounded-xl ">
          <img alt="mern" loading="lazy" decoding="async" className=" min-h-64 object-cover rounded-md" style={{ color: "transparent", width: "100%", height: "100%" }} src={blog?.img} />
        </div>
        <h3 className="text-lg md:text-[20px] font-medium mt-10 md:mt-12 lg:mt-16">{blog?.title}</h3>
        <p className="text-gray-500 text-base mt-2"></p>
        <div className="text-gray-500 text-base mt-2" dangerouslySetInnerHTML={{ __html: truncatedContent }} />
      </div>
    </Link>
  );
};
